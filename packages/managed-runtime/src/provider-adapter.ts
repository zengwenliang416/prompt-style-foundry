...
export interface GenerateRequest {
  ...
  inputImage: Buffer;
  inputMime: string;
  /** Optional cooperative cancellation for Worker graceful shutdown. */
  signal?: AbortSignal;
}
...
export type ProviderOutcome = { ok: true; value: NormalizedGeneration } | NormalizedFailure;
...
export class ProviderAdapter {
  ...
  async generate(request: GenerateRequest): Promise<ProviderOutcome> {
    ...
    const multipart = createMultipartBody(request);
    ...
    if (first === undefined || typeof first.b64_json !== 'string') {
      ...
      return {
        ok: false,
        code: 'PROVIDER_REJECTED',
        message: 'provider result missing image data',
      };
    }
...
function createMultipartBody(request: GenerateRequest): { body: Blob; contentType: string } {
  const boundary = `onepic-${randomBytes(18).toString('hex')}`;
  const parts: Buffer[] = [];
  ...
  parts.push(
    bytes(
      `--${boundary}\r\nContent-Disposition: form-data; name="image"; filename="${inputFilename(request.inputMime)}"\r\n` +
        `Content-Type: ${request.inputMime}\r\n\r\n`,
    ),
    request.inputImage,
    bytes(`\r\n--${boundary}--\r\n`),
  );
  ...
  return {
    body: new Blob([new Uint8Array(Buffer.concat(parts))]),
    contentType: `multipart/form-data; boundary=${boundary}`,
  };
}

function inputFilename(mime: string): string {
  if (mime === 'image/jpeg') return 'input.jpg';
  if (mime === 'image/webp') return 'input.webp';
  return 'input.png';
}
...
export function credentialFreeMessage(message: string): string {
  return message.includes('Bearer') || message.includes('sk-')
