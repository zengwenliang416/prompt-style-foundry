#!/usr/bin/env python3
"""Create the deterministic, allowlisted standalone public artifact (O06)."""
...
ALLOWED = (
    re.compile(r"^(?:index\.html|favicon\.svg)$"),
    re.compile(r"^assets/(?:app\.js|fx\.js|styles\.css|vendor/anime\.esm\.min\.js)$"),
...
def public_files() -> list[tuple[Path, str]]:
    files: list[tuple[Path, str]] = []
    for path in sorted(PUBLIC.rglob("*")):
        if path.is_symlink():
            fail(f"Symlink is not allowed in public artifact: {path.relative_to(ROOT)}")
        if path.is_dir():
            continue
        if not path.is_file():
            fail(f"Non-regular public entry is not allowed: {path.relative_to(ROOT)}")
        relative = path.relative_to(PUBLIC).as_posix()
        if not any(pattern.fullmatch(relative) for pattern in ALLOWED):
            fail(f"Public artifact path is not allowlisted: {relative}")
        data = path.read_bytes()
        if path.suffix.lower() in {".html", ".svg", ".js", ".css", ".json", ".txt"}:
            if SECRET.search(data):
                fail(f"Credential-like content found in public artifact file: {relative}")
...
def main() -> None:
    if len(sys.argv) != 2:
        fail("Usage: scripts/create_static_artifact.py /path/to/output.tar.gz.tmp")
    output = Path(sys.argv[1]).resolve()
    output.parent.mkdir(parents=True, exist_ok=True)
    files = public_files()
    if not files or not (PUBLIC / "index.html").is_file():
        fail("public artifact is empty or missing index.html")
    mtime = int(os.environ.get("SOURCE_DATE_EPOCH", "0"))
    members = files + list(LICENSES)
    for source, name in members:
        if not source.is_file() or source.stat().st_size == 0:
            fail(f"Required artifact source is missing or empty: {source.relative_to(ROOT)}")
    ...
    print(f"Created deterministic artifact with {len(members)} files: {output}")
